...
// Console baud rate, generated by `build.rs` from `ESP_CSI_CLI_UART_BAUD` (default 115200).
// Defines `UART_BAUD: u32`.
//
// A build-time const rather than a runtime setting: a command that changed the console's own baud
// would have to answer at a rate the host has already stopped listening at. See `build.rs`.
include!(concat!(env!("OUT_DIR"), "/uart_baud.rs"));
...
///
/// `with_peer_mac` switches off automatic magic-prefix pairing in favour of explicit per-node peer
/// filtering; `with_ht40` forces the per-peer TX PHY to HT40. Both are applied only when the user
/// configured them.
...
/// pair, whose PHY rate is fixed by the fast profile rather than taken from `set-rate`.
...
async fn main(spawner: Spawner) -> ! {
    ...
    let serial = {
        ...
        #[cfg(feature = "esp32")]
        {
            Uart::new(
                peripherals.UART0,
                esp_hal::uart::Config::default().with_baudrate(UART_BAUD),
            )
                .unwrap()
...
async fn csi_collection(
    ...
    loop {
        ...
        // frames in the next run.
        esp_csi_rs::set_csi_peer_filter(user_config.csi_peer_filter.unwrap_or([0u8; 6]));
        esp_csi_rs::set_csi_min_sig_mode(user_config.csi_min_sig_mode);
        ...
        // WiFi infra modes — an emitter's inject loop and the ESP-NOW pair's own
        // exchange are neither of them the ICMP flood.
